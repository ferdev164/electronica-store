      ...
      WHERE p.activo = true
        AND (
          ...
          ${palabras.length > 1 ? Prisma.sql`
            OR (${Prisma.join(likePalabras.map(lp => Prisma.sql`p.nombre ILIKE ${lp}`), ' AND ')})
          ` : Prisma.empty}
        )
        AND (${categoriaFiltro ? Prisma.sql`c.slug = ${categoriaFiltro}` : Prisma.sql`1=1`})
